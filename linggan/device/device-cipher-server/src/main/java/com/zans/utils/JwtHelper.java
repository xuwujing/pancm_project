...
public class JwtHelper {
    ...
    public String signUser(SysUser sysUser) {
        ...
        return JWT.create().withHeader(header)
                ...
                .withClaim(SESSION_KEY_IS_ADMIN, sysUser.getIsAdmin())
...
//            log.info("jwt token#{} -> {}", token, jwt.getExpiresAt());
            ...
            session.setIsAdmin(Integer.parseInt(getClaimAsString(jwt, SESSION_KEY_IS_ADMIN) == null ? "0" : getClaimAsString(jwt, SESSION_KEY_IS_ADMIN)));
            return session;
        } catch (Exception e) {
