...
import com.zans.base.config.HttpsClientRequestFactory;
...
//        httpRequestFactory.setReadTimeout(readTimeoutSeconds * 1000);
    ...
    public RestTemplate getHttpsDefaultRestTemplate() {
        return getHttpsDefaultRestTemplate(60, 60);
    }
...
//        httpRequestFactory.setReadTimeout(readTimeoutSeconds * 1000);
    ...
    public JSONObject getForObject2(String url, Map<String, ?> uriVariables) {
        RestTemplate restTemplate = getDefaultRestTemplate();
        JSONObject result = null;
        try {
            result = restTemplate.getForObject(url, JSONObject.class, uriVariables);
        } catch (Exception ex) {
            log.error("getForObject error#" + url, ex);
        }
        restTemplate = null;
    ...
    public ApiResult postHttpsForObject(String url, Map<String, Object> postMap) {
        log.info("请求接口的url地址:{},请求参数:{}",url,JSONObject.toJSONString(postMap));
        RestTemplate restTemplate = getHttpsDefaultRestTemplate();
        ApiResult result = null;
        try {
    ...
    public ApiResult postForObject(String url, Map<String, Object> postMap,int connectTimeoutSeconds, int readTimeoutSeconds) {
        RestTemplate restTemplate = getDefaultRestTemplate(connectTimeoutSeconds,readTimeoutSeconds);
        ApiResult result = null;
        try {
            MultiValueMap<String, String> map= new LinkedMultiValueMap<String, String>();
            if (postMap != null && postMap.size() > 0) {
    ...
    public boolean accessSuccess(ApiResult result) {
