...
import com.zans.mms.dao.DevicePointSubsetDetailMapper;
import com.zans.mms.dao.DevicePointSubsetMapper;
...
import com.zans.mms.vo.devicepoint.DevicePointExportVO;
...
public class DevicePointSubsetDetailServiceImpl extends BaseServiceImpl<DevicePointSubsetDetail> implements IDevicePointSubsetDetailService {
	 ...
	 */
	@Override
	public String exportFile(Long subsetId,String filName) {
		String subsetName=devicePointSubsetMapper.getSubsetNameById(subsetId);
		//写文件逻辑 制定导出模板
		List<String> rowNameList = new ArrayList<>();
		Collections.addAll(rowNameList,"子集名称","点位编码","点位名称","辖区名称","路口类型","设备类型","取电方式","通讯接入方式","经度","维度","负责单位");
		...
		//再查询所有的巡检子集导出信息
		List<DevicePointExportVO> devicePointList= devicePointMapper.getExportListBySubsetId(subsetId);
		writeData(absoluteNewFilePath,devicePointList,subsetName);
		//开始写数据
	...
	public String export(String subsetId, String fileName) {
		//写文件逻辑 制定导出模板
		List<String> rowNameList = new ArrayList<>();
		Collections.addAll(rowNameList,"子集名称","点位编号","点位名称","辖区名称","路口类型","设备类型","取电方式/位置","通讯接入方式","经度","纬度","所属单位及设备类型");
		//此时写完文件头
		String absoluteNewFilePath=null;
		try {
...
//"id","点位编号","点位名称","辖区名称","路口类型","设备类型","取电方式/位置","通讯接入方式","经度","纬度","所属单位及设备类型");
	 ...
	 * @param devicePointList 数据列表
	 */
	public void writeData(String absoluteNewFilePath, List<DevicePointExportVO> devicePointList,String subsetName) {
		FileInputStream fs=null;
		FileOutputStream out =null;
		try{
			//获取absoluteNewFilePath
