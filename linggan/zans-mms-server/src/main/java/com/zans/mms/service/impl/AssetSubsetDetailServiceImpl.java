...
import com.zans.mms.dao.AssetMapper;
import com.zans.mms.dao.AssetSubsetDetailMapper;
...
public class AssetSubsetDetailServiceImpl extends BaseServiceImpl<AssetSubsetDetail> implements IAssetSubsetDetailService {
	...
	/**
	 * 资产子集中资产数据导出
	 * @param assetSubsetDetaiQueryReqVO 子集id
	 * @param fileName 文件名称
	 * @return
	 */
	@Override
	public String export(AssetSubsetDetaiQueryReqVO assetSubsetDetaiQueryReqVO, String fileName) {
		if(null==assetSubsetDetaiQueryReqVO.getSubsetId()){
			throw new BusinessException("资产子集id传参为空！");
		}
		String subsetName=assetSubsetDetailMapper.getSubsetNameById(assetSubsetDetaiQueryReqVO.getSubsetId().toString());
		//写文件逻辑 制定导出模板
		List<String> rowNameList = new ArrayList<>();
		Collections.addAll(rowNameList,"子集名称","子集设备类型","设备编号","点位编号","点位名称","设备方位","设备功能","ip","mac地址","子网掩码",
				"网关","在线状态","设备序列号","设备型号","设备品牌","项目名称","建设年份","建设单位","建设单位联系人","建设单位联系电话","运维单位",
				"运维单位联系人","运维单位联系电话","检测方式","维护状态","设备账号","设备密码","安装时间","车道数","备注","负责单位");
		...
		int pageNum = assetSubsetDetaiQueryReqVO.getPageNum();
		int pageSize = Integer.MAX_VALUE;
		Page page = PageHelper.startPage(pageNum, pageSize);
		List<AssetExportVO> assetExportVOList= assetMapper.getExportData(assetSubsetDetaiQueryReqVO);
		this.writeAssetData(absoluteNewFilePath,assetExportVOList,subsetName);
		return absoluteNewFilePath;
	}
	 ...
	 */
	@Override
	public void clearById(Long subsetId) {
		assetSubsetDetailMapper.clearById(subsetId);
	}
	 ...
	 */
	private void writeAssetData(String absoluteNewFilePath, List<AssetExportVO> assetExportVOList,String subsetName) {
		...
		try{
			...
			for(int i =0;i<assetExportVOList.size();i++){
				...
				row.createCell(currentCell++).setCellValue(subsetName);
				row.createCell(currentCell++).setCellValue(assetExportVOList.get(i).getDeviceType());
				...
				//11/17添加 在线状态
				row.createCell(currentCell++).setCellValue(assetExportVOList.get(i).getOnlineStatus());
