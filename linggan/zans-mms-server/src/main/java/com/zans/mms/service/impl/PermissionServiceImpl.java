...
import com.zans.mms.dao.SysPermissionMapper;
...
//    @Cacheable(cacheNames = "PERM_ROUTE_MAP", key = "#userId")
    ...
    @Override
    public List<PermissionRespVO> findPermissionByRole(String jurisdictionId, Integer moduleType) {
        return permissionMapper.findPermissionByRole(jurisdictionId,moduleType);
    }
    ...
    public void updateRolePermission(RoleEditVO vo) {
        this.permissionMapper.deletePermissionByRole(vo.getJurisdictionId(),vo.getModuleType());
        for (Integer permId : vo.getCheckedPermission()) {
            this.permissionMapper.insertRolePerm(vo.getJurisdictionId(), permId);
        }
        for (SysRolePermVO perm : vo.getCheckedDataPerm()) {
            ...
            permissionMapper.updateDataPerm(vo.getJurisdictionId(),perm.getPermId(), dataPermNum, dataPerm);
        }
        permissionHelper.removeRoleDataPerm(vo.getJurisdictionId());
        permissionHelper.removeRoleRoute(vo.getJurisdictionId());
        permissionHelper.removeRoleMenu(vo.getJurisdictionId());
    }
    ...
    public DataPermVO getDataPermByUserAndPermId(UserSession userSession, Integer permId) {
        DataPermCacheVO permCacheVO = permissionHelper.getRoleDataPerm(userSession.getJurisdictionId() , permId);
        if (null == permCacheVO ){
            List<DataPermCacheVO> rolePermList = this.findRolePermListByRoleId(userSession.getJurisdictionId());
            permissionHelper.setRoleDataPerm(rolePermList);
            permCacheVO = permissionHelper.getRoleDataPerm(userSession.getJurisdictionId() , permId);
        }
...
//        vo.setDataPermDesc(permCacheVO.getDataPermDesc());
    ...
    @Override
    public List<SysRolePerm> findDataPermList(String jurisdictionId) {
        RolePermVO vo = new RolePermVO();
        vo.setJurisdictionId(jurisdictionId);
    ...
    public List<String> getAllFrontRouteByUser(SysUser user) {
        List<String> roleRoute = permissionHelper.getRoleRoute(user.getJurisdictionId());
        if (null == roleRoute || roleRoute.size()==0) {
            List<String> routes = this.getAllFrontRouteByUserId(user.getId());
            permissionHelper.setRoleRoute(user.getJurisdictionId(),routes);
            return routes;
        }
        return roleRoute;
    }
    ...
    public List<MenuItem> getMenuTreeByUser(SysUser user, int permCacheYes, Integer moduleType) {
        List<MenuItem> menus = permissionHelper.getRoleMenuItem(user.getJurisdictionId()+moduleType);
        if (null == menus || menus.size()==0){
            List<MenuItem> menuItems = this.getMenuTreeByUserId(user.getId(), permCacheYes, moduleType);
            permissionHelper.setRoleMenuItem(user.getJurisdictionId()+moduleType,menuItems);
            return menuItems;
        }
        return menus;
    }
    ...
    public DataPermVO getTopDataPerm(UserSession userSession) {
        ...
        if(StringHelper.equals(ROLE_MAINTAION,baseOrgRepVO.getOrgType())||StringHelper.equals(ROLE_BUILD,baseOrgRepVO.getOrgType())){
    ...
    private final String ROLE_BUILD = "02";


    @Override
    public List<TreeSelect> findRolePermissionInTree(String jurisdictionId, Integer moduleType) {
        List<SysModule> menuList = moduleService.findMenuList(moduleType) ;
        ...
        for (PermissionRespVO vo : permList) {
            ...
            if (1 == vo.getDataPermStatus().intValue()){
                ...
                defineList = sysDataPermDefineService.getListByCondition(new HashMap<String,Object>(){{
                    ...
                    put("jurisdictionId",jurisdictionId);
                }});
            }
