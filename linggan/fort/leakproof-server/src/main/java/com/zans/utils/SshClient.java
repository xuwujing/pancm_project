...
import static com.zans.constant.SystemConstant.SEPARATOR_LINE;
...
public class SshClient {
    ...
    public static String stopAndStartService(String host, String account, String password, Integer port, String projectName,Integer startStopFlag) {
        String startStopStr = " start ";
        if (startStopFlag.intValue()==0){
            startStopStr = " stop ";
        }
        String stopCommand = " supervisorctl "+startStopStr +projectName;
        String[] stopCommands = {
                stopCommand};
        String msg = execShell(host, account, password, port, stopCommands, true);
        if (msg == null) {
            return null;
        }
        String[] lines = msg.split("\n");
        ...
        List<Object> configList = new LinkedList<>();
        for (String line : lines) {
            if (line.contains(stopCommands[3])) {
                start = true;
            }
            if ("return".equals(line)) {
                break;
    ...
    public static void main(String[] args) throws Exception {
        Boolean root = collet("root", "Admin#12$34!", "192.168.2.4", 22, 5000);
        System.out.println(root);
//        stopAndStartService("192.168.2.4","root", "Admin#12$34!",22,"scan");
