...
import com.zans.dao.SysConstantMapper;
import com.zans.model.SysConstant;
import com.zans.vo.Pair;
...
public class GlobalConstants {
    ...
    public static Map<String, Pair> AGENT_SERVER = new HashMap<>();
...
//    @Value("${api.upload.folder}")
    ...
    private void init(){
        ...
        List<SysConstant> decodeInfo = sysConstantMapper.getDecodeInfo();
        log.info("decodeInfo:{}",decodeInfo);
        decodeMap = new HashMap<>();
        for (SysConstant sysConstant : decodeInfo) {
            decodeMap.put(sysConstant.getConstantKey(),sysConstant.getConstantValue());
        }
        log.info("deocodeMap:{}",decodeMap);
